...
class WindowGenerator:
    def __init__(
        self, input_width, label_width, shift, train_df, val_df, test_df, label_columns=None, output_labels=None
    ):
        ...
        if self.output_labels is not None:
            ...
            if first_output_key in df_keys:
                ...
                self.experts_output_label_slices = [
                    slice(0, exp_output_idx),
                    slice(132, 144),
                ]  # ! currently all features : to update later
                for i in self.experts_output_label_slices:
                    print(f"target features for the {i}'th slice are: {df_keys[i]}")
    ...
    def split_window(self, input_data):
        print(f"type(input_data): {type(input_data)}, shape: {tf.shape(input_data)}")
        ...
        experts_labels_check = [
            input_data[:, self.labels_slice, data_slice] for data_slice in self.experts_output_label_slices
        ]

        experts_labels = tf.concat(experts_labels_check, -1)  # to check

        print(f"type(experts_labels): {type(experts_labels)}, shape: {tf.shape(experts_labels)}")
        print(f"type(experts_labels): {type(experts_labels)}, shape: {tf.shape(experts_labels)}")

        if self.label_columns is not None:
            gate_labels = tf.stack(
                [gate_labels[:, :, self.column_indices[name]] for name in self.label_columns], axis=-1
            )
            experts_labels = tf.stack(
                [experts_labels[:, :, self.column_indices[name]] for name in self.label_columns], axis=-1
            )
