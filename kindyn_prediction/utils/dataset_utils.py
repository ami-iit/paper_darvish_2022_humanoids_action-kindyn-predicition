...
def plot_action_recognition_prediction(plt_, prediction, labels, prediction_time_idx):
    ...
    for i in range(len(prediction_time_idx)):
        plt.bar(
            ...
            width=width,
            color=colors[i],
            label=f"t + {prediction_time_idx[i] * sampling_time} [sec]",
        )
        x = x + width
        ...
        for n in range(max_n):
            ...
            for i in range(np.shape(self.output_time)[0]):
                if True:  # self.output_time[i][-1] > time:
                    self.axs_motion[n].scatter(
                        self.output_time[i],
                        self.output_prediction[i],
                        marker="X",
                        edgecolors="k",
                        label="Predictions",
                        c="#ff7f0e",
    ...
    def motion(self, time, inputs, prediction, plot_indices, plot_columns):
        ...
        for n in range(max_n):
            ...
            for i in range(np.shape(self.output_time)[0]):
                if True:  # self.output_time[i][-1] > time:
                    alpha = i / total_data
                    self.axs_motion[n].plot(
                        self.output_time[i], self.output_prediction[i], color=(alpha, 0.2, 0.2, alpha)
                    )
                    self.axs_motion[n].scatter(
                        self.output_time[i][0],
                        self.output_prediction[i][0],
                        marker="o",
                        color=(alpha, 0.2, 0.2, alpha),
                        zorder=2,
                    )
                    self.axs_motion[n].scatter(
                        self.output_time[i][-1],
                        self.output_prediction[i][-1],
                        marker="o",
                        color=(alpha, 0.2, 0.2, alpha),
                        zorder=2,
                    )
